...
const iconMap: IconMapType = {
  'JavaScript': (_, size) => <SiJavascript className={`text-[#F7DF1E] ${size}`} />,
  'TypeScript': (_, size) => <SiTypescript className={`text-[#3178C6] ${size}`} />,
  'React': (_, size) => <SiReact className={`text-[#61DAFB] ${size}`} />,
  'Next.js': (_, size) => <SiNextdotjs className={`text-gray-900 dark:text-white ${size}`} />,
  'Node.js': (_, size) => <SiNodedotjs className={`text-[#339933] ${size}`} />,
  'Express': (_, size) => <SiExpress className={`text-gray-900 dark:text-white ${size}`} />,
  ...
  'Django Rest Framework': (selected, size) => {
    const sizeMap: Record<'text-lg' | 'text-xl' | 'text-2xl' | 'text-3xl' | 'text-4xl', string> = {
      'text-lg': 'w-5 h-5',
      'text-xl': 'w-6 h-6',
      'text-2xl': 'w-7 h-7',
      'text-3xl': 'w-8 h-8',
  ...
  'PostgreSQL': (_, size) => <SiPostgresql className={`text-[#4169E1] ${size}`} />,
  ...
  'Tailwind CSS': (_, size) => <SiTailwindcss className={`text-[#06B6D4] ${size}`} />,
  'Bootstrap': (_, size) => <SiBootstrap className={`text-[#7952B3] ${size}`} />,
  'Git': (_, size) => <SiGit className={`text-[#F05032] ${size}`} />,
  'Github': (_, size) => <SiGithub className={`text-gray-900 dark:text-white ${size}`} />,
  ...
  'TensorFlow': (_, size) => <SiTensorflow className={`text-[#FF6F00] ${size}`} />,
...
const TechBadge: React.FC<TechBadgeProps> = ({
  ...
  return (
    ...
    >
      {getIcon && (
        <span className={`flex items-center justify-center ${variant === 'compact' ? 'mb-2' : ''}`}>
