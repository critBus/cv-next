...
const ProjectsSection: React.FC<ProjectsSectionProps> = ({ projects }) => {
  ...
  const filteredProjects = useMemo(() => {
    if (selectedTechnologies.length === 0 && selectedPositions.length === 0)
      return projects;
  ...
  const totalFilteredProjects = useMemo(() => {
    return Object.values(filteredProjects).reduce(
      (acc, projectList) => acc + projectList.length,
      0
    );
  }, [filteredProjects]);
  ...
  return (
    <section className="w-full">
      ...
      <div
        ref={filtersRef}
        className="mb-12 bg-gray-800 rounded-xl p-6 shadow-md"
      >
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-gray-400">
              {totalFilteredProjects} proyecto
              {totalFilteredProjects !== 1 ? "s" : ""} encontrado
              {totalFilteredProjects !== 1 ? "s" : ""}
            </span>
          </div>
        </div>
      </div>
